        ...
        public:
            ...
            class Sync_queue_count{
                private: 
                    ...
                    //per distruttore in Threadpool
                    void set_stop_to_true_and_notify(){
        ...
        public:
            ...
            ~Threadpool(){
                ...
                for(int j = 0; j<n_worker_; j++){
                    sync_queues_[j]->set_stop_to_true_and_notify();
                }
                for(int j = 0; j<n_worker_; j++){
                    threads_[j].join();
                }
            ...
            void worker_loop(int i){
                while(!stop_){
                    sync_queues_[i]->wait(*this); //oss: no if(stop) tanto male che va finisce iterazione facendo pop di nullopt 
            ...
            int get_count_job_all(){
                int count = 0;
                for(int i=0; i<n_worker_; i++){
                    count += sync_queues_[i]->get_count_job();
                }
                return count;
            }
            ...
            bool send_task_round(job j){
                ...
                bool flag = sync_queues_[indxw_.indx_]->push_back(j);
                notifica_tutti();
                loc.unlock();
                if(flag){
