            ...
            void worker_loop(int i){
                ...
                while(!workers_[i]->stop_){
                    ...
                    if(try_j){//esegue se non è nullopt
                        ...
                        bool has_job = (count_job_[i].load(std::memory_order_acquire) > 0); //has_job dentro mutex cosi che lettura di cout_job[] sincronizzata con i count_job[]++ dei send
                        workers_[i]->cv_.wait(loc,[&](){return has_job || get_count_all_job()>0 || workers_[i]->stop_ ;}); //oss: spostato get_count_job() per primo cosi si riduce chiamata a empty() che blocca push e pop su coda. //OSS: magari get_cout_jo()>N cosi da evitare sveglia se steal non necessario (logica da coordinare con send)
                        loc.unlock();
