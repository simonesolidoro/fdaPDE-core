    ...
    template <steal T> class Threadpool{
        ...
        public:
           ...
           //send_task_mostfree
            ...
            //send che invia un job che wrappa una reference ad una funzione e non una copia
            //per evitare di copiare stessa body_function da passare ai job ogni volta, si puo fare perche in parallel_for non viene distrutta funzioje a cui si fa la reference finche tutti i job non vengono eseguiti (cioè finche non si fa il get di tutti i future)
            std::optional<std::future<void>> send_task_round_ref_parallel_for(std::function<void(int)>& f,int i) {
                std::shared_ptr<std::packaged_task<void()>> ptr_task = std::make_shared<std::packaged_task<void()>> ([&f,i]()mutable{return f(i);});
                std::future<void> fut = ptr_task->get_future();
                job j = [ptr_task](){(*ptr_task)();}; //non serve mutable perche ptr è copia const ma puo chiamare membri non const (operator ()) di oggetto puntato (packaged_task)
            ...
            void parallel_for_sure(int start, int end, F&& f){
                ...
                ret_fut.reserve(end-start);
                int j = start;
                std::function<void(int)> fun = std::forward<F>(f);
                while(j<end){
                    std::optional<std::future<return_type>> opt_fut= this->send_task_round_ref_parallel_for(std::ref(fun),j);
                    if(opt_fut){
                        ret_fut.push_back(std::move(opt_fut.value()));
                        j++;
                    }
            ...
            void parallel_for_sure(int start, int end,int n, F&& f){
                ...
                int j = 0;
                std::function<void(int)> fun = std::forward<F>(f); //cosi da passare referece ad unica body function e non doverla copiare ad ogni iterazione. oss:qui sed_task_roud perche poi copia di lamda che fa wrap di sottoisiemi di for non è un prolema perchè ogniuna usa ref a body_fun per singola iterazione
                while(j< n){
                    std::optional<std::future<return_type>> opt_fut = this->send_task_round([&fun,start, j, n_body_fun]()mutable{ //j catturato come copia perchè modificato detro job (j+1) quidi se catturi come reference si sballa tutto !!!
                        for(int k=j*n_body_fun+start; k<(j+1)*n_body_fun+start; k++ ){
                            fun(k);
                            //std::cout<<"iterazione "<<k<<" di for"<<std::endl;
                        }
                    });
                    if(opt_fut){
                ...
                //OSS: se n>range -> unico job di for completo (come sequetiale), non mettiamo if per controllo valore di n chi lo usa deve assicurarsi che sia tra (1,range)
                if(range % n > 0){ //inviamo ultimo job con iterazioni rimanenti. //oss: > e non !=0 perchè cosi anche qui si evita di verificare che range sia positivo, in altri è fatto implicitamete in primo while: j=start while(j<end)...
                    j=0;
                    while(j<1){
                        std::optional<std::future<return_type>> opt_fut = this->send_task_round([&fun,n, end,start, n_body_fun]()mutable{ //j gia catturata in & credo non serve
            ...
            void parallel_for_sure(int start, int end,std::vector<int>& vect, F&& f){
                ...
                std::function<void(int)> fun = std::forward<F>(f);
                while(j< vect_size){
                    std::optional<std::future<return_type>> opt_fut = this->send_task_round([&fun, j, seq]()mutable{ //j gia catturata in & credo non serve
