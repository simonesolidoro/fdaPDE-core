...
# test_for.txt 
rm -f "test_for_sbilanciati.txt"
touch "test_for_sbilanciati.txt"

# Esegui il programma N volte e salva l'output
for i in $(seq 1 "$1"); do
    ./"test_for_sbilanciati" "$2" "$3" "$4" "$5" "1" "$6">> "test_for_sbilanciati.txt" 
done
...
for nb in "${blocchi[@]}"; do
    ...
    output_file="test_parallel_for_sure_n_sbilanciati${nb}.txt"
    rm -f "$output_file"
    touch "$output_file"

    # Esegui il programma N volte e salva l'output
    for i in $(seq 1 "$1"); do
        ./"test_parallel_for_sure_n_sbilanciati" "$2" "$3" "$4" "$5" "$nb" "$6">> "$output_file" 
    done
done
