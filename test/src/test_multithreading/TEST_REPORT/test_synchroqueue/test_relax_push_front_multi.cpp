...
void push_front_di_n_elem(fdapde::Synchro_queue<value,fdapde::relax> & q,int n, value el){
    for (int j=0; j<n; j++){
        q.push_front(el);
    }
};
...
int main(int argc, char** argv){
    ...
    fdapde::Synchro_queue<value,fdapde::relax> q1(size_coda);
    value el = "ciao";

//push_back() multithreading

    fdapde::Synchro_queue<value,fdapde::relax> q(size_coda);
